...
service_name_new_query = ""
require_A_record = False
require_TXT_record = False
require_SRV_record = False
require_PTR_record = False
waiting_for_records = False
...
class DNS_SD_GUI:
    ...
    def __init__(self, root):
        ...
        self.services_frame = ttk.LabelFrame(self.root, text="Servicii disponibile")  # Titlul secțiunii
        ...
        self.service_listbox.pack(fill="both", expand=True, padx=10, pady=10)

        # Buton pentru a actualiza Listbox-ul
        self.refresh_button = ttk.Button(self.root, text="Refresh", command=self.refresh_listbox)
        self.refresh_button.pack(pady=10)

        # Adauga un eveniment de selectie pentru a obtine serviciul selectat
        self.service_listbox.bind('<<ListboxSelect>>', self.on_service_selected)

        # Frame pentru widget-ul de afisare a_data si txt_data cu titlu
        self.details_frame = ttk.LabelFrame(self.root, text="Detalii serviciu selectat", relief="solid", borderwidth=2)
    ...
    def refresh_listbox(self):
        ...
        with sqlite3.connect("rcpDB_try1.db") as conn:
            ...
            if row[0] == 0:
                ...
                # Trimite query pentru descoperirea serviciilor
                query_packet = bytes.fromhex(
            ...
            cursor.execute(
                "SELECT service_name, a_data, txt_data FROM services WHERE instance_name IS NULL")  # instance_name NULL -> in inregistrare apare doar numele serviciului, fara instante
            rows = cursor.fetchall()

            # Adauga randurile in Listbox
            for row in rows:
                service_name = row[0]  # Service Name va fi afisat pe interfata
    ...
    def on_service_selected(self, event):
        ...
        service_name = self.service_listbox.get(selected_index[0])  # Obtine numele serviciului selectat
        # Verifica daca exista inregistrari complete pentru serviciul selectat
        if not self.check_service_in_db(service_name):
    ...
    # Interogheaza baza de date pentru a verifica daca exista o inregistrare completa
    def check_service_in_db(self, service_name):

        # Numele de serviciu este uneori salvat cu punct la final, alteori fara. De aceea, verificam existenta in BD a serviciului cu cele doua variante posibile ale sale
        new_service_name = service_name + '.'

        # verificare serviciu cu punct
        self.cursor_db_gui.execute('''SELECT A_DATA, TXT_DATA FROM services WHERE SERVICE_NAME = ?''',
                                   (new_service_name,))

        print("new_service_name in check_servicedb", new_service_name)
        rows = self.cursor_db_gui.fetchall()

        for row in rows:
            # daca query-ul sqlite returneaza ceva. Daca da, se verifica validitatea/existenta inregistrarilor a si txt
            if row is not None and row[0] is not None and row[1] is not None:
                print("Row", row)
                self.service_data = row
                return True  # Exista o inregistrare completa

        # daca este service_name fara punct la final
        self.cursor_db_gui.execute('''SELECT A_DATA, TXT_DATA FROM services WHERE SERVICE_NAME = ?''',
                                   (service_name,))
    ...
    def decode_mdns_data(self, cursor_receive, conn_receive):
        ...
        if arcount == 0:  # Verificam numarul de records adiționale
            ...
            for _ in range(ancount):  # Iteram prin toate raspunsurile
                ...
                if answer_type == 0xC:  # PTR Record
                    ...
                    while rdata_offset < rdlength:  # parcurgem rdata
                        if (rdata[
                                rdata_offset] & 0xC0) == 0xC0:  # Verificare daca avem un pointer reprezentat prin 2 octeti primii fiind '11', nume de domeniu comprimat
                            ...
                            while self.data[pointer_temp_offset] != 0:  # iterare pana la terminator de sir
                                ...
                                if (
                                        label_length & 0xC0) == 0xC0:  # Pointer intr-un pointer (rar, dar posibil) -> Merge doar daca pointer-ul e la final
                                    pointer_temp_offset = int.from_bytes(
                                        self.data[pointer_temp_offset:pointer_temp_offset + 2], 'big') & 0x3FFF
                                else:
                                    pointer_temp_offset += 1  # skip lungime
                                    pointer_data = self.data[
                                                   pointer_temp_offset:pointer_temp_offset + label_length].decode(
                                        'utf-8')
                            ...
                            service_name += rdata[rdata_offset:rdata_offset + label_length].decode(
                                'utf-8')  # decodificam eticheta
            ...
            name = self.data[offset:offset + 2]  # primii doi biti din eticheta
            ...
            for answ in range(total_answer_count):
                ...
                if answer_type == 0x01: #nu iteram, deoarece o inregistrare poate avea doar rdlength=4 pt cei 4 octeti de IP
                    ...
                    # Formatează adresa IP din cei 4 octeți
                    ip_address = ".".join(map(str, a_data))  # Transformă octeții în string și concatenează cu punct
                    # Afișează adresa IP
                    print("A record")
                    print(ip_address)
